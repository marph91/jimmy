...
class Converter(converter.BaseConverter):
    ...
    def convert_note(self, chat):
        ...
        note_imf = imf.Note(
            title, source_application=self.format, original_id=str(chat["id"])
        )

        note_body = []
        for message in chat["messages"]:
            if message["type"] != "service" and message.get("action") == "create_group":
                note_imf.created = common.timestamp_to_datetime(
            ...
            if (file_ := message.get("file")) is not None:
                if content:
                    content += "\n"
                file_md = (
                    f"![{message.get('file_name', '')}]({str(self.root_path / file_)})"
                )
                note_imf.resources.append(
                    imf.Resource(
                        self.root_path / file_, file_md, message.get("file_name")
                    )
                )
                content += file_md
    ...
    def convert(self, file_or_folder: Path):
        input_json = json.loads(
            (file_or_folder / "result.json").read_text(encoding="utf-8")
        )
